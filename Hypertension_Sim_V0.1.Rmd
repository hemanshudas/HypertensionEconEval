...
v.n <- c("W","MI","ST","cD","D")        #Model States: W-Well, CVD-CVD Event, MI - Myocardial Infraction, ST-Stroke, cD - CVD Death, D - Death due to other causes
...
p.W_CVD <- 0.0245                  #Probability of CVD transitory event when well; 50 yr, M, Smoker, SBP 150mmHg, BMI 27.7
p.CVD_MI <- 0.0521                 #Probability of MI when CVD event occurs
p.CVD_ST <- 0.9479                 #Probability of Stroke when CVD event occurs
p.MI_cD <- 0.17                    #Probability of CVD death when MI occurs
p.ST_cD <- 0.15                    #Probability of CVD death when Stroke occurs
p.W_D <- p.MI_D <- p.ST_D <- 0.00978 #Probability of normal death from any state

#Intiatlization of Cost for each state
c.W <- 0                      #cost of being well
c.MI <- 2857                  #cost of MI treatment (Assumed to be INR 2,00,000)
c.ST <- 4304                  #cost of Stroke treatment (Assumed to be INR 3,00,000)
c.D <- c.cD <- 0              #cost of Death

#Intialization of Utility for each state
u.W <- 1                      #utility of being well
u.MI <- 0.76                  #utility of MI state
u.ST <- 0.629                 #utility of Stroke state
u.D <- u.cD <- 0              #utility of death

# If not treated
p.W_CVD_NT <- 0.0245          #15% reduction in CVD risk
c.W_NT <- 0                   #Cost of medication and consultation in a year

# If treated
p.W_CVD_T <- 0.0208           #15% reduction in CVD risk
c.W_T <- 54.85                #Cost of medication and consultation in a year
...
m.CS <- matrix(nrow = 3, ncol=11,
               dimnames = list(c("Well","CVD Death","Death"),
                               paste("Cycle",0:10,sep = " ")))
m.CS[,] <- 0 
...
results <- list(m.M = m.M, m.C = m.C, m.U = m.U, count_cD = count_cD, tc_hat = tc_hat, tu_hat = tu_hat, tu=tu, TS = TS, TR = TR, count_CVD = count_CVD, m.CS = m.CS)
return(results)
...
Probs <- function(M.it,p.W_CVD) {
          ...
          v.p.it[M.it == "W"] <- c(1-(p.W_CVD+p.W_D),p.W_CVD*p.CVD_MI,p.W_CVD*p.CVD_ST,0,p.W_D) #tranistion probability when well
          v.p.it[M.it == "MI"] <- c(0,1-(p.MI_cD+p.MI_D),0,p.MI_cD,p.MI_D)       #transition probability when in MI state
          v.p.it[M.it == "ST"] <- c(0,0,1-(p.ST_cD+p.ST_D),p.ST_cD,p.ST_D)                           #transition probability when in Stroke state
          v.p.it[M.it == "cD"] <- c(0,0,0,1,0)                     #transition probability when in CVD Death state
          v.p.it[M.it == "D"] <- c(0,0,0,0,1)                       #transition probability when in Death state
...
v.CVD <- c(sim_control$count_CVD,sim_treat$count_CVD)
v.C <- c(sim_control$tc_hat,sim_treat$tc_hat)
v.U <- c(sim_control$tu_hat,sim_treat$tu_hat)
v.cD <-c(sim_control$count_cD,sim_treat$count_cD)
...
colnames(table_microsim) = c("CVD Events","CVD Deaths","Costs($)", "QALYs", "ICER") # name the columns
